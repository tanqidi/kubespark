...
// Handler handles Kubernetes resource requests
type Handler struct {
	resourcesOperator resources.Interface
	onResourceCreated func(created any)
}
...
// SetOnResourceCreated sets a callback that will be called when a resource is created
func (h *Handler) SetOnResourceCreated(callback func(created any)) {
	h.onResourceCreated = callback
}
...
func (h *Handler) ListNamespaces(req *restful.Request, resp *restful.Response) {
	opts := utils.ParseListOptions(req)
...
func (h *Handler) CreateResourceByGVR(req *restful.Request, resp *restful.Response) {
	...
	if h.onResourceCreated != nil {
		h.onResourceCreated(created)
	}
...
func (h *Handler) GetNamespaceResources(req *restful.Request, resp *restful.Response) {
	...
	result, err := h.resourcesOperator.ListResources(req.Request.Context(), namespace, resource, utils.ParseListOptions(req))
	if err != nil {
		h.handleError(resp, err)
		return
	}
...
func (h *Handler) handleError(resp *restful.Response, err error) {
	if statusErr, ok := err.(interface{ Status() metav1.Status }); ok {
		kapis.WriteErrorWithCode(resp, int(statusErr.Status().Code), int(statusErr.Status().Code), err.Error())
		return
	}
